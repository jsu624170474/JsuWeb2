...
public class PaginatorTag extends TagSupport{
	private final Logger logger = LoggerFactory.getLogger(PaginatorTag.class); 
...
//					buffer.append("<input type=\"button\" class=\"last\" value=\"尾&nbsp;&nbsp;页\" disabled=\"disabled\" />");
					...
					int calIndex2=totalPage-(paginator.getCurrentPage()+(paginator.getShowPage()-1)/2);
				...
				buffer.append(" <strong>  <input type='hidden'  name='currentPage' class=\"page-skip\" value='"+ paginator.getCurrentPage() + "'onkeyup=\"value=value.replace(/^[^1-9]+|[^\\d]/g,'');\" onblur='checkPage(document.all.currentPage.value,document.all.pageSize.value,"+paginator.getTotalCount()+")'/>");
				buffer.append(" <strong> 到第 <input type='text'  name='pagetag' class=\"page-skip\" value=''onkeyup=\"value=value.replace(/^[^1-9]+|[^\\d]/g,'');\" onblur='checkPage(document.all.currentPage.value,document.all.pageSize.value,"+paginator.getTotalCount()+")'/>页</strong>");
